                                  ...
                                  <div className="px-5 pb-4 bg-primary/[0.02] border-t border-primary/10">
                                    ...
                                    {(() => {
                                      ...
                                      for (const it of s.items) {
                                        ...
                                        if (!groups[key]) { groups[key] = { shop: (it.shop_name || '?') + (it.round_number ? ' #' + it.round_number : ''), desc: it.description || it.item_type, members: [], qty: 0, inclusions: 0, weight_g: 0 }; seenClaimsByGroup[key] = new Set() }
                                        ...
                                        // inclusions_count is per-claim total, not per-row — a claim split across several
                                        // members saves it onto every resulting row identically, so each distinct claim
                                        // (by claim_group_id, when the row has one) is only added once, not once per row.
