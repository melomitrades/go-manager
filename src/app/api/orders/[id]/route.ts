...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await query('ALTER TABLE order_items ADD COLUMN IF NOT EXISTS inclusions_count INTEGER DEFAULT 0').catch(() => {})
  await query('ALTER TABLE order_items ADD COLUMN IF NOT EXISTS entries_count INTEGER DEFAULT 0').catch(() => {})
  await query('ALTER TABLE orders ADD COLUMN IF NOT EXISTS is_multi_version BOOLEAN DEFAULT false').catch(() => {})
  await query('ALTER TABLE orders ADD COLUMN IF NOT EXISTS version_names JSONB DEFAULT NULL').catch(() => {})
  await query('ALTER TABLE orders ADD COLUMN IF NOT EXISTS version_options JSONB DEFAULT NULL').catch(() => {})
  await query('ALTER TABLE order_items ADD COLUMN IF NOT EXISTS price_krw NUMERIC(12,0)').catch(() => {})
  ...
  const items = await query(`
    ...
    WHERE oi.order_id = $1
    ${isGom ? '' : `AND oi.joiner_id = '${user.id}'`}
  ...
  const paidRows = await query(
    'SELECT joiner_id, paid, paid_at FROM order_joiner_paid WHERE order_id=$1',
    [params.id]
  ).catch(() => [] as any[])
